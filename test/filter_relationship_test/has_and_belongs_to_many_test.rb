...
class HABTMTest < ActiveSupport::TestCase
  ...
  test '::filter :habtm => [INT]' do
    ...
    r2 = create(:region)
    r3 = create(:region)
    p1 = create(:property, :regions => [r2])
    p2 = create(:property, :regions => [r1, r3])

    assert_equal [p1, p2].map(&:id),  Property.filter(:regions => [r1.id, r2.id]).map(&:id)
    assert_equal [p2, p2].map(&:id),  Property.filter(:regions => [r1.id, r3.id]).map(&:id) # TODO.. not sure about this, p2 is duped
    assert_equal(<<-SQL.strip.gsub(/\s+/, ' '), Property.filter(:regions => [1,2]).to_sql.strip.gsub('"', ''))
      SELECT properties.* FROM properties
      INNER JOIN properties_regions ON properties_regions.property_id = properties.id
      INNER JOIN regions ON regions.id = properties_regions.region_id
